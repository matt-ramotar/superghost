...
class UpdateViewModel: ObservableObject {
    ...
    static func userFacingErrorMessage(for error: Swift.Error) -> String {
        ...
        if let networkError = networkError(from: nsError) {
            ...
            case NSURLErrorNotConnectedToInternet:
                return String(
                    localized: "update.error.noInternet.shared.message",
                    defaultValue: "This app can’t reach the update server. Check your internet connection and try again."
                )
            case NSURLErrorTimedOut:
            ...
            case NSURLErrorCannotConnectToHost:
                return String(
                    localized: "update.error.serverUnreachable.shared.message",
                    defaultValue: "This app couldn’t connect to the update server. Check your connection or try again later."
                )
            case NSURLErrorNetworkConnectionLost:
        ...
        if nsError.domain == SUSparkleErrorDomain {
            ...
            case 2001:
                return String(
                    localized: "update.error.feedDownload.shared.message",
                    defaultValue: "This app couldn’t download the update feed. Check your connection and try again."
                )
            case 1000, 1002:
            ...
            case 1003, 1005, 4005:
                return String(
                    localized: "update.error.permissionError.shared.message",
                    defaultValue: "Move this app into Applications and relaunch to enable updates."
                )
            default:
                break
            }
    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = ReleaseIdentity.releaseTagURL(tag: tag) {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let url = ReleaseIdentity.commitURL(hash: newHash) {
                self = .commit(url)
            } else {
                return nil
            }
